...
namespace GraphViz
{
	...
	public:
		...
		void print() {	std::cout << prop << "=\"" << value << "\", "; }
	};
	...
	public:
		...
		{
			std::cout << name << " [";
			for (size_t p(0); p<props.size(); ++p) { props[p].print(); }
			std::cout << "]\n";
		}
	};
	...
	public:
		...
		void print()
		{
			...
			for (size_t p(0); p<props.size(); ++p) { std::cout << props[p].prop << "=\"" << props[p].value << "\"\n"; }
			for (size_t n(0); n<nodes.size(); ++n) { nodes[n].print(); }
			for (size_t e(0); e<edges.size(); ++e) { edges[e].print(); }
			std::cout << "}\n";
		}
	};

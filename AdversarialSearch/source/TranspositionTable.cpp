...
TTEntry::TTEntry(const HashType & hash2, const AlphaBetaScore & score, const size_t & depth, const int & type, 
...
const AlphaBetaScore & TTEntry::getScore()						const { return _score; }
...
TranspositionTable::TranspositionTable () 
	: TT(Search::Constants::Transposition_Table_Size, TTEntry())
	, size(Search::Constants::Transposition_Table_Size)
    , collisions(0)
    , lookups(0)
    , found(0)
    , notFound(0)
...
const size_t TranspositionTable::getSaveIndex(const size_t & index, const HashType & hash2, const size_t & depth) const
{
	...
	for (size_t i(index); (i<(index+Search::Constants::Transposition_Table_Scan)) && (i <TT.size()); ++i)
	{
...
void TranspositionTable::save(	const HashType & hash1, const HashType & hash2, const AlphaBetaScore & value, const size_t & depth, const int & type,
			const IDType & firstPlayer, const AlphaBetaMove & bestFirstMove, const AlphaBetaMove & bestSecondMove)
{
